...
RUN yum -y --setopt=tsflags=nodocs install libXrender libXext  fontconfig libfontenc libXfont urw-fonts \
 && yum clean all \
 &&  https://github.com/wkhtmltopdf/wkhtmltopdf/releases/download/0.12.4/wkhtmltox-0.12.4_linux-generic-amd64.tar.xz      | tar -C /usr/local --strip-components 1 -xJ && chown -R root.root /usr/local/{lib,include,share,bin} 
